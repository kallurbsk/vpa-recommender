...
func makeTestUsageSample() *ContainerUsageSampleWithKey {
	return &ContainerUsageSampleWithKey{ContainerUsageSample{
		...
		testContainerID, int32(restartCount)}
}
...
func TestClusterAddSample(t *testing.T) {
	...
	assert.NoError(t, cluster.AddOrUpdateContainer(testContainerID, testRequest, restartCount, ctrReady, ctrCurState))
...
func TestClusterGCAggregateContainerStateDeletesOldFailedPod(t *testing.T) {
	// Create a pod with a single container.
	cluster := NewClusterState()
	...
	cluster.GarbageCollectAggregateCollectionStates()

	// AggegateContainerState should not be deleted as the pod is still active.
	assert.NotEmpty(t, cluster.aggregateStateMap)
	assert.NotEmpty(t, vpa.aggregateContainerStates)

	cluster.Pods[pod.ID].Phase = apiv1.PodFailed
	cluster.GarbageCollectAggregateCollectionStates()
...
func TestMissingKeys(t *testing.T) {
	...
	err = cluster.AddOrUpdateContainer(testContainerID, testRequest, restartCount, ctrReady, ctrCurState)
	assert.EqualError(t, err, "KeyError: {namespace-1 pod-1}")
}
...
func addNonRunningPod(cluster *ClusterState) *PodState {
	cluster.AddOrUpdatePod(testPodID, testLabels, apiv1.PodFailed)
	return cluster.Pods[testPodID]
}

func addTestContainer(cluster *ClusterState) *ContainerState {
	cluster.AddOrUpdateContainer(testContainerID, testRequest, restartCount, ctrReady, ctrCurState)
...
func TestTwoPodsWithSameLabels(t *testing.T) {
	...
	cluster.AddOrUpdateContainer(containerID1, testRequest, restartCount, ctrReady, ctrCurState)
	cluster.AddOrUpdateContainer(containerID2, testRequest, restartCount, ctrReady, ctrCurState)
...
func TestEmptySelector(t *testing.T) {
	...
	assert.NoError(t, cluster.AddOrUpdateContainer(containerID1, testRequest, restartCount, ctrReady, ctrCurState))
	...
	assert.NoError(t, cluster.AddOrUpdateContainer(containerID2, testRequest, restartCount, ctrReady, ctrCurState))
	...
	for _, tc := range cases {
		t.Run(tc.name+", VPA first", func(t *testing.T) {
			...
			for _, podDesc := range tc.pods {
				...
				assert.NoError(t, cluster.AddOrUpdateContainer(containerID, testRequest, restartCount, ctrReady, ctrCurState))
			}
